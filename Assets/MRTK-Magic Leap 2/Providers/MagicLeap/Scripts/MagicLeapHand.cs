...
namespace MagicLeap.MRTK.DeviceManagement.Input
{
    ...
    public class MagicLeapHand : BaseHand
    {
        ...
        private float airPinchAngle = 0.15f;
        ...
        private void CalculatePinch()
        {
            ...
            if (useMLGestureClassification && gesturesStarted)
            {
                ...
                if (GestureClassification.TryGetHandPosture(_gestureHand, out GestureClassification.PostureType postureType) && postureType == GestureClassification.PostureType.Pinch)
                {
                    //Use Specific Pose instead of posture if able to get pose.
                    if(GestureClassification.TryGetHandKeyPose(_gestureHand, out GestureClassification.KeyPoseType keyPoseType))
                    {
                        if(keyPoseType == GestureClassification.KeyPoseType.Pinch || keyPoseType == GestureClassification.KeyPoseType.OK)
                        {
                ...
                // air-tap/pinch
                else if(GestureClassification.TryGetFingerState(_gestureHand, GestureClassification.FingerType.Index, out GestureClassification.FingerState indexFingerState) && indexFingerState.PostureData.PinchNormalAngle <= airPinchAngle)
                {
                    isPinchingGesture = true;
                }
            }
